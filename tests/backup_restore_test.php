...
final class backup_restore_test extends \advanced_testcase {
    ...
    /**
     * Test that a backup/restore without existing feedback still preserves config.
     */
    public function test_backup_and_restore_config_only(): void {
        global $CFG, $DB, $USER;
        ...
        $env = $this->create_test_environment();
        $this->create_aif_config($env, 'Config-only prompt', 0);
        ...
        $newconfigs = $DB->get_records('assignfeedback_aif', ['assignment' => $newassign->id]);
        $this->assertCount(1, $newconfigs);
        $this->assertSame('Config-only prompt', reset($newconfigs)->prompt);

        // No feedback rows because no grade existed.
        $this->assertFalse($DB->record_exists('assignfeedback_aif_feedback', ['aif' => reset($newconfigs)->id]));
    }
}
